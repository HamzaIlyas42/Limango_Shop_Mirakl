...
**Before generating any `{Prefix}_Connection`, `{Prefix}_Integration`, or `{Prefix}_Integration_Test` classes from a markdown or spec file, you MUST ask the user:**
...
- **{Prefix}\_Connection**: Invocable-only wrapper for Flow/Process Builder. No HTTP logic. Delegates to {Prefix}\_Integration.
- **{Prefix}\_Integration**: Holds all HTTP logic, Named Credentials, validation, and callouts. Used by Connection and by other Apex.
...
- **Methods**:
    - `@InvocableMethod(Don't use lable, description='...', category='...')` → `callXxxApi(List<InvocableInput> inputs)` → converts to Input, calls `executeRequests`, maps to InvocableOutput
...
- Keep the same structure, method names, and logic as the reference LIM_Mirakl_Integration and LIM_Mirakl_Connection; only names and comments change.
